...
metaphor migration generate Invoice recruitment
metaphor migration run
...
(`recruitment` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed recruitment          # run Rust seeders
metaphor migration generate-seeds recruitment  # emit SQL seed files
```
