//! Domain policy for JobApplication aggregate
//!
...
/// Domain policy for JobApplication — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<JobApplication>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type JobApplicationDomainPolicy = PermitAllPolicy<JobApplication>;
