...
/// Create stateless routes for the Recruitment module.
///
...
///
/// ```ignore
/// let module = RecruitmentModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::RecruitmentModule) -> Router<()> {
    Router::new()
        .merge(create_candidate_routes(module.candidate_service.clone()))
        .merge(create_interview_routes(module.interview_service.clone()))
        .merge(create_job_application_routes(module.job_application_service.clone()))
        .merge(create_job_offer_routes(module.job_offer_service.clone()))
        .merge(create_job_requisition_routes(module.job_requisition_service.clone()))
}

/// Read-only routes for the Recruitment module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_recruitment_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_recruitment_routes(module: &crate::RecruitmentModule) -> Router<()> {
    Router::new()
        .merge(create_candidate_read_routes(module.candidate_service.clone()))
        .merge(create_interview_read_routes(module.interview_service.clone()))
        .merge(create_job_application_read_routes(module.job_application_service.clone()))
        .merge(create_job_offer_read_routes(module.job_offer_service.clone()))
        .merge(create_job_requisition_read_routes(module.job_requisition_service.clone()))
}

/// Get all routes (stateless) for the Recruitment module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::RecruitmentModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Recruitment module.
///
...
pub fn create_combined_routes(module: &crate::RecruitmentModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Recruitment module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::RecruitmentModule) -> Router<AppState> {
    Router::new()
